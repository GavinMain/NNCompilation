#Version: MLP_01 adds learning rate, which improves accuracy and also correlates the loss and accuracy values
import numpy as np
import time
import torch
...
class MLP:
  ...
  def forward(self, input_data):
      ...
      input_data = self.hidden_layer.forward(input_data)
      input_data = relu(input_data)
  ...
  def backward(self, gradient, learning_rate):
      ...
      gradient = gradient * relu_gradient(self.hidden_layer.out)
      gradient = self.hidden_layer.backward(gradient, learning_rate)
      gradient = gradient * relu_gradient(self.input_layer.out)
      gradient = self.input_layer.backward(gradient, learning_rate)

def train_model(model, train_data, train_labels, lr=lr):
  out = model.forward(train_data)
  predictions = softmax(out)
  loss = cross_entropy_loss(predictions, train_labels)
  gradient = cross_entropy_loss_gradient(predictions, train_labels)
...
if __name__ == "__main__":
  ...
  model = MLP(input_size=28 * 28, hidden_size=256, output_size=10)
